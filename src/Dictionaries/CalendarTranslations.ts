  ...
  calendar: {
    ...
    interaction: {
      ...
      'en': {
        ...
        adminNotOwner: 'This command is reserved for the bot owner.',
        adminInvalidId: 'That doesn\'t look like a Discord id — pick a suggestion from the list.',
        adminNoGuilds: 'I\'m not in any servers.',
        adminNoEvents: 'No active events in that server.',
        adminLeaveConfirm: 'Leave **{guild}**? Its native scheduled events are removed and its bot events close first.',
        adminLeaveConfirmLabel: 'Leave server',
        adminLeaveDone: 'Left the server. Closed {count} event(s) on the way out.',
        adminLeaveAlready: 'I was already out of that server — cleaned up {count} leftover event(s) locally.',
        adminLeaveOwned: 'I own that server, so I can\'t leave it. Events were closed anyway.',
        adminDetachDone: 'Channel detached: {count} event(s) closed, {mirrors} Events-tab entr(y/ies) removed. New events there are blocked until you reattach.',
        adminDetachAlready: 'That channel is already detached.',
        adminReattachDone: 'Channel reattached (was {state}) — new events are allowed there again. Previously closed events stay closed.',
        adminReattachNone: 'That channel wasn\'t detached or quarantined.',
      },
    },
    scheduledEvent: {
      $filter: 'lang',
